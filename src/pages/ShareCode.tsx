...
import { ChevronLeft, Lock, Copy, Check, Loader2 } from 'lucide-react'
...
export default function ShareCode() {
  ...
  const handleCopy = () => {
    if (shareToken?.token) {
      navigator.clipboard.writeText(shareToken.token)
      setCopied(true)
    ...
    <DashboardLayout showProfile={false}>
      <div className="flex flex-col gap-6 sm:gap-8 pb-10">
        ...
        <div className="flex items-center gap-4 sm:gap-6">
          ...
          <h1 className="text-[18px] sm:text-[20px] font-bold text-black">Access Code</h1>
        </div>
        ...
        {step === 1 ? (
          ...
          <div className="flex flex-col items-center gap-6 sm:gap-10 pt-4 sm:pt-10">
            <div className="flex flex-col items-center gap-6 sm:gap-8 text-center bg-white p-6 sm:p-8 rounded-xl border border-[#f5f6f7] shadow-sm w-full">
              <div className="w-14 h-14 sm:w-16 sm:h-16 bg-purple-accent rounded-full flex items-center justify-center text-white">
                <Lock className="w-7 h-7 sm:w-8 sm:h-8" />
              </div>
              <div className="flex flex-col gap-2">
                <h2 className="text-[16px] sm:text-[18px] font-bold text-black">Access Code Generated!</h2>
                <p className="text-[13px] sm:text-[14px] text-[#8d8989]">Share this code with authorized personnel.</p>
              </div>
              <div className="flex items-center justify-center gap-3 bg-[#f5f6f7] p-4 sm:p-6 rounded-lg w-full">
                <span className="text-[24px] sm:text-[32px] font-bold text-navy-dark tracking-wider">
